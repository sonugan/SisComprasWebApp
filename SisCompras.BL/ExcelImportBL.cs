...
namespace SisCompras.BL
{
    ...
    //
    // https://www.codeproject.com/Articles/32370/Import-Excel-File-to-DataSet
    //

    public class ExcelImportBL
    {
        //public static DataSet ImportExcelXLS(HttpPostedFile file, bool hasHeaders)
        public DataSet ImportExcelXLS(HttpPostedFileBase file, bool hasHeaders)
        {

            string fileName = Path.GetTempFileName();
            file.SaveAs(fileName);

            return ImportExcelXLSFromFile(fileName, hasHeaders);
        }

        public DataSet ImportExcelXLSFromFile(string FileName, bool hasHeaders)
        {
            string HDR = hasHeaders ? "YES" : "NO";
            //string strConn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + FileName + ";Extended Properties=\"Excel 11.0;HDR=" + HDR + ";IMEX=1\"";
            string strConn= "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + FileName + ";Extended Properties=\"Excel 12.0 Xml; HDR = " + HDR +"\" "; 
                ...
                if (rows.Count > 0) {
                    ...
                    for (int i = startIndex; i < rows.Count; i++) {
                        ...
                        for (int cellIndex = 0; cellIndex < cells.Count; cellIndex++) {
                            ...
                            if (actualCellIndex >= dt.Columns.Count) {
                                for (int iCol = dt.Columns.Count; iCol < actualCellIndex; iCol++) {
